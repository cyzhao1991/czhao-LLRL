...
def main(gpu_num, exp_num, env = None, **kwargs):
	...
	with tf.device('/gpu:%i'%(gpu_num)):
		...
		actor_net = Fcnn(sess, pms.obs_shape, pms.action_shape, [64,64], name = pms.name_scope, if_bias = [True], activation = ['tanh', 'tanh', 'None'], init = [1. ,1., .01])
		actor = GaussianActor(actor_net, sess, pms)

		baselinet_net = Fcnn(sess, pms.obs_shape, 1, [64,64], name = 'baseline', if_bias = [True], activation = ['tanh','tanh','None'], init = [1. ,1., .1])
