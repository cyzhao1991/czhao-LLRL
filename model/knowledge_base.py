...
class KnowledgeBase(object):
	...
	def __init__(self, layer_shape, num_of_latent, layer_type = 'fcnn', name = 'KB'):
		...
		with tf.name_scope(name):
			for i in range(self.num_of_layer):
				...
				for j in range(self.num_of_latent):
					self.L_weights[i].append( tf.Variable(tf.truncated_normal([n_in, n_out], stddev = 1.0), name = 'L_weights_layer%i_latent%i'%(i, j), trainable = True) )
					self.L_bias[i].append( tf.Variable(tf.truncated_normal([n_out], stddev = 1.0), name = 'L_bias_layer%i_latent%i'%(i,j), trainable = True) )
...
class PathPolicy(object):
	...
	def __init__(self, input_dim, output_dim, knowledge_base = None, layer_shape = None, num_of_latent = None, \
		...
		if allow_context:
			...
			self.input, self.output, self.context = self.build(name = name, pre_defined_context = pre_defined_context)
			self.target_input, self.target_output, self.target_context = self.build(name = name + '_target', pre_defined_context = pre_defined_context)
		else:
			self.input, self.output = self.build(name = name, pre_defined_context = pre_defined_context)
			self.target_input, self.target_output = self.build(name = name + '_target', pre_defined_context = pre_defined_context)

	def build(self, name, pre_defined_context = None):
		
		with tf.name_scope(name):
			inputs = tf.placeholder(tf.float32, [None, self.input_dim], name = 'input')
			self.input_weights = tf.Variable( tf.truncated_normal([self.input_dim, self.layer_shape[0]], stddev = 1.0), name = 'weights_input', trainable = True)
			self.input_bias = tf.Variable( tf.truncated_normal([self.layer_shape[0]], stddev = 1.0), name = 'bias_input', trainable = True)
			...
			if self.allow_context:
				...
				self.context_weights = tf.Variable( tf.truncated_normal([self.context_dim, self.layer_shape[self.context_layer]], stddev = 1.0), name = 'weights_context',  trainable = True)
				self.context_bias = tf.Variable( tf.truncated_normal([self.layer_shape[self.context_layer]], stddev = 1.0), name = 'bias_context', trainable = True)
				context_out = tf.nn.relu( tf.matmul(contexts, self.context_weights) + self.context_bias)
			for i in range(self.num_of_layer):
				self.s_weights.append( tf.Variable( tf.truncated_normal([self.num_of_latent], stddev = 1.0), name = 's_weights_layer%i'%(i), trainable = True) )
				self.s_bias.append( tf.Variable( tf.truncated_normal([self.num_of_latent], stddev = 1.0), name = 's_bias_layer%i'%(i), trainable = True) )
