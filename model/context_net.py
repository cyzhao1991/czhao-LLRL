...
class MoeNet(Net):
	def __init__(self, sess, input_dim, output_dim, layer_dim, module_num, name = None, **kwargs):
		super(Moe_Net, self).__init__(sess, input_dim, output_dim, layer_dim, name, **kwargs)
		self.module_num = module_num
		self.num_of_hidden_layer = len(self.layer_dim)
		self.num_of_layer = self.num_of_hidden_layer + 1
		
		self.def_Shared_knowledge(self.name)
		self.def_Gate_knowledge(self.name)
		self.input = tf.placeholder(tf.float32, [None, input_dim])
		self.output, self.all_net, self.all_h, self.all_g = self.build(self.name)
	...
	def def_Gate_knowledge(self, name):

		with tf.name_scope(name):
			self.gate_weights = [tf.Variable(tf.truncated_normal([input_dim, m_n], stddev = 1.), dtype = tf.float32, \
				name = 'gate_w_h%i'%k) for k, m_n in enumerate(self.module_num)]
			self.gate_bias = [tf.Variable(tf.truncated_normal([m_n], stddev = 1.), dtype = tf.float32, \
				name = 'gate_b_h%i'%k) for k, m_n in enumerate(self.module_num)]


	def build(self, name):
		net = [self.input]
		all_h = [None] * self.num_of_layer
		all_g = [None] * self.num_of_layer
		with tf.name_scope(name):
			for i in range(self.num_of_layer):
				all_h[i] = [tf.activation_fns_call[i](net[i] @ w + b) for w,b in zip(self.KB_weights[i], self.KB_bias[i])]
				all_g[i] = tf.sigmoid( 3.*(self.input @ self.gate_weights[i] + self.gate_bias[i]) )
				net.append( tf.reduce_sum( tf.stack(all_h[i], axis = 2) @ tf.expand_dims(all_g[i], axis = -1), axis = -1) )
		return net[-1], net, all_h, all_g
