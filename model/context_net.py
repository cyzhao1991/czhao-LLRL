...
class Context_Fcnn_Net(Net):
	...
	def def_Task_knowledge(self, name):
		with tf.name_scope(name):
			...
			self.context = tf.placeholder(tf.float32, [None, self.context_dim], name = 'context')
			self.c_weights = [tf.Variable(tf.truncated_normal([self.context_dim, dim], stddev = 1.0), \
				name = 's_vector_h%i'%k) for dim,k in zip(self.module_num, range(self.num_of_layer))]
			self.s_vector = [tf.matmul(self.context, w_c) for w_c in self.c_weights]
