...
filename_list = ['new_Data/dm_control/stl/walker_s%1.1f/w%1.1fg0.0/exp0/shelve_result'%(i,j) for i in s_list for j in [0]]
...
for y in all_result['average_return']:
	y = np.array([np.mean(y[np.max([0, i-10]):i+1]) for i in range(1000)])
