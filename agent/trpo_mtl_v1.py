...
class TRPO_MTLagent(Agent):
	...
	def init_vars(self):
		with tf.name_scope(self.pms.name_scope):
			...
			surr_grad_task_list = [tf.gradients(surr_loss, self.shared_var_list + task_var) for surr_loss, task_var in zip(self.total_loss_list, self.task_var_list)]
			self.flat_surr_grad_task_list = map(flatten_var, surr_grad_task_list)
	...
	def train_paths(self, all_paths):
		...
		l1_norm_list = self.sess.run(self.l1_norm)
		stats = dict(
