...
class MtlMimicAgent(Agent):
	...
	def init_vars(self):
		...
		self.l1_loss = 0.0 * tf.add_n( [tf.reduce_sum(tf.abs(v)) for v in self.s_var_list] )
		self.column_loss = 0.0 * tf.add_n( [tf.reduce_sum( tf.reduce_max( tf.abs(v), axis=0 )) for v in self.s_var_list] ) 
