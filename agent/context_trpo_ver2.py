...
class Context_TRPO_Agent(Agent):
	...
	def init_vars(self):
		...
		with tf.name_scope(self.pms.name_scope):
			...
			self.pg_grad = tf.gradients( -self.pg_loss , self.var_list) 
			self.sparse_grad = tf.gradients( self.l1_norm + self.column_loss, self.var_list )
			self.flat_pg_grad = flatten_var( [pg+0 if sg is None else pg+sg for pg,sg in zip(self.pg_grad, self.sparse_grad) ] )
			...
			self.var_flat_mask = tf.concat([self.shared_flat_mask,self.task_flat_mask], axis = 0)
	...
	def train_paths(self, paths):
		...
		feed_dict = {self.obs: obs_source[inds],
					 ...
					 self.old_dist_mean: np.array([a_info['mean'] for a_info in actor_info_source[inds]]),#[:,np.newaxis],
					 self.old_dist_logstd: np.array([a_info['logstd'] for a_info in actor_info_source[inds]])
					 }
		...
		task_pg_gradients = np.where( self.sess.run(self.var_flat_mask), task_pg_gradients, 0)

		def loss_function(x):
			self.sess.run( self.set_var_from_flat, feed_dict = {self.weights_to_set: x })
			_, kl = self.sess.run( [self.surr_loss, self.kl], feed_dict = feed_dict	)
			return 0, kl
