...
class Context_TRPO_Agent(Agent):
	...
	def init_vars(self):
		...
		with tf.name_scope(self.pms.name_scope):
			...
			if self.pms.contextual_method is 'meta_s_network':
				self.total_loss = self.surr_loss + self.pms.l1_regularizer * tf.maximum(16.,self.l1_norm) #/ self.old_l1_norm
			elif self.pms.contextual_method is 'concatenate':
	...
	def update_vars(self, shared_var_list, task_var_list):
		...
		with tf.name_scope(self.pms.name_scope):
			...
			surr_grad = tf.gradients(self.total_loss, var_list)
			self.flat_surr_grad = flatten_var(surr_grad)
	...
	def get_single_path(self, task_context):
		...
		rewards = []
		actor_infos = []
		grav_context = 3.* task_context + self.default_context
		if self.pms.env_name.startswith('walker'):
			# print('gravity', grav_context)
			self.env.physics.model.opt.gravity[0] = grav_context[0]
			self.env.physics.model.opt.gravity[1] = grav_context[1]
	...
	def train_paths(self, paths):
		...
		# shared_step_gradient = cg(lambda x: fisher_vector_product(x, 'shared'), -g[:self.shared_var_num], cg_iters = self.pms.cg_iters)
		...
		# task_step_gradient = cg(lambda x: fisher_vector_product(x, 'task'), -g[self.shared_var_num:], cg_iters = self.pms.cg_iters)
		...
		def loss_function(x, grad_vars = None):
			if grad_vars is None:
				...
				return self.sess.run([self.surr_loss, self.l1_norm, self.kl], feed_dict = feed_dict)
		...
		if self.pms.linesearch:
			...
			# flat_task_new = linesearch_new( lambda x: loss_function(x, grad_vars = 'task'), flat_task_prev, task_fullstep_gradient, self.pms.max_backtracks, self.pms.max_kl)
			# flat_theta_new = np.concatenate([flat_shared_new, flat_task_new], axis = 0)
		else:
