...
dir_name = 'Data/dm_control/mtl_prog/mtl_walker_s%1.1f/w0.0g0.0/exp0/'%SPEED
...
with tf.device('/gpu:%i'%(gpu_num)):
	...
	# sess.run(tf.global_variables_initializer())000
# mimic_agent.learn(all_obs, all_con, all_acs)
saver = tf.train.Saver(max_to_keep = 100)
# model_name = 'Data/mimic_data/multi_wind_mimic_0.ckpt'
model_name = 'Data/dm_control/stl/walker_s0.0/w0.0g0.0/exp0/walker-iter500.ckpt'
...
shared_w_varlist = [v for v in tf.trainable_variables() if 'shared_ws' in v.name]
sess.run([tf.assign(var, value) for var, value in zip(shared_w_varlist, pre_learned_var)])
shared_b_varlist = [v for v in tf.trainable_variables() if 'shared_bs' in v.name]
sess.run([tf.assign(var, tf.zeros(var.shape)) for var in shared_b_varlist])
